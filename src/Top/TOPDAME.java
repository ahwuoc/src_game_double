...
public class TOPDAME {
    ...
    public void load() {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement(
                "SELECT * FROM player ORDER BY  CAST(JSON_EXTRACT(data_point, '$[10]') as unsigned) DESC limit 100"); ResultSet rs = ps.executeQuery()) {
    ...
    private Player processPlayerResultSet(ResultSet rs) throws SQLException {
        ...
        JSONArray dataObject = (JSONArray) jv.parse(rs.getString("data_point"));
        player.nPoint.power = Double.parseDouble(String.valueOf(dataObject.get(1)));
    ...
    private void extractDataPoint(String dataPoint, Player player) {
        ...
        player.nPoint.dame = Double.parseDouble(dataArray.get(11).toString());
        dataArray.clear();
    }
