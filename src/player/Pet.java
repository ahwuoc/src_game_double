...
public class Pet extends Player {
    ...
    private static final short ARANGE_CAN_ATTACK = ConstPet.ATTACK_RANGE_CAN_ATTACK;
    private static final short ARANGE_ATT_SKILL1 = ConstPet.ATTACK_RANGE_SKILL1;
    ...
    private static final int TIME_WAIT_AFTER_UNFUSION = ConstPet.TIME_WAIT_AFTER_UNFUSION;
    ...
    public String getStrLevel() {
        ...
        double clevel = (this.nPoint.power - cap.getPower()) * 10000 / maxPower;
    ...
    private String getTextStatus(byte status) {
        if (this.typePet == ConstPet.TYPE_BLACK) {
            switch (status) {
                case FOLLOW:
    ...
    public void fusion2(boolean porata) {
     ...
     public void hopthebongtai(boolean porata,byte typeBONGTAI) {
        if (this.isDie()) {
            Service.gI().sendThongBao(master, "Yêu cầu phải có đệ tử và đệ tử còn sống");
            return;
        }
        if (Util.canDoWithTime(lastTimeUnfusion, TIME_WAIT_AFTER_UNFUSION)) {
            if (porata) {
                master.fusion.typeFusion = typeBONGTAI;
            }
            this.status = FUSION;
            ChangeMapService.gI().exitMap(this);
            fusionEffect(master.fusion.typeFusion);
    ...
    private void moveIdle() {
        ...
        if (idle && Util.canDoWithTime(lastTimeMoveIdle, timeMoveIdle)) {
            ...
            timeMoveIdle = Util.nextInt(ConstPet.TIME_MOVE_IDLE_MIN, ConstPet.TIME_MOVE_IDLE_MAX);
            idle = false;
        }
    ...
    private void masterDoesNotAttack() {
        if (Util.canDoWithTime(master.lastTimePlayerNotAttack, master.timeNotAttack)) {
            ...
            master.timeNotAttack = Util.nextInt(ConstPet.TIME_MASTER_NOT_ATTACK_MIN,
                    ConstPet.TIME_MASTER_NOT_ATTACK_MAX); // random 30p - 1h
        }
    }
    ...
    public void update() {
        try {
            if (this.master != null && this.master.zone != null) {
                ...
                if (isDie()) {
                    if (System.currentTimeMillis() - lastTimeDie > ConstPet.TIME_DIE_RESPAWN) {
                ...
                switch (status) {
                    ...
                    case PROTECT:
                        ...
                        if (playerAttack != null) {
                            if ((this.typePet == ConstPet.TYPE_BERUS || this.typePet == ConstPet.TYPE_BLACK)
                                    && Util.isTrue(1, 5)
                                    && playerAttack.nPoint.hp < ConstPet.HP_THRESHOLD_HAKAI
        ...
        if (Util.canDoWithTime(lastTimeAskPea, 10000)) {
            if (this.master.isPet) {
                if (this != null && !this.isDie()) {
                    int statima = ConstPet.STAMINA_RECOVERY;
                    long hpKiHoiPhuc = ConstPet.HP_MP_RECOVERY;
    ...
    public short getHead() {
        ...
        if (effectSkill != null && effectSkill.isMonkey) {
            ...
            return 412;
        } else if (this.typePet == ConstPet.TYPE_MABU && !this.isTransform) {
            return 297;
        } else if (this.typePet == ConstPet.TYPE_BERUS && !this.isTransform) {
            return 508;
        } else if (this.typePet == ConstPet.TYPE_PIC && !this.isTransform) {
            return 237;
        } else if (this.typePet == ConstPet.TYPE_XENCON && !this.isTransform) {
            return 264;
        } else if (this.typePet == ConstPet.TYPE_BROLY && !this.isTransform) {
            return 294;

        } else if (this.typePet == ConstPet.TYPE_BLACK_ALT && !this.isTransform) {
            return 1909;
        } else if (this.typePet == ConstPet.TYPE_BLACK_ROSE && !this.isTransform) {
            return 1910;
        } else if (this.typePet == ConstPet.TYPE_ZAMASU && !this.isTransform) {
            return 1506;
        } else if (this.typePet == ConstPet.TYPE_CUMBER && !this.isTransform) {
            return 1204;

        } else if (this.typePet == ConstPet.TYPE_BLACK && !this.isTransform) {
            if (!this.isTransform) {
                return 550;
            }
            return 553;
        ...
        if (this.nPoint.power < ConstPet.POWER_SPECIAL_PETS) {
    ...
    public short getBody() {
        ...
        if (effectSkill != null && effectSkill.isMonkey) {
            ...
            return 238;
        } else if (this.typePet == ConstPet.TYPE_BLACK) {
            return 880;
        } else if (this.typePet == ConstPet.TYPE_XENCON && !this.isTransform) {
            return 265;
        } else if (this.typePet == ConstPet.TYPE_BROLY && !this.isTransform) {
    ...
    private void updatePower() {
        if (this.playerSkill != null) {
            switch (this.playerSkill.getSizeSkill()) {
                case 1:
                    if (this.nPoint.power >= ConstPet.POWER_SKILL2) {
                        openSkill2();
                    }
                    break;
                case 2:
                    if (this.nPoint.power >= ConstPet.POWER_SKILL3) {
                        openSkill3();
                    }
                    break;
                case 3:
                    if (this.nPoint.power >= ConstPet.POWER_SKILL4) {
                        openSkill4();
                    }
                    break;
                // case 4:
    ...
    public void transform() {
        if (this.typePet == ConstPet.TYPE_MABU) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Bố Mày Là Bư Nè !! Bư..Bư..Bư..Ma..Nhân..Bư....");
        }
        if (this.typePet == ConstPet.TYPE_BERUS) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
        ...
        if (this.typePet == ConstPet.TYPE_XENCON) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }
        if (this.typePet == ConstPet.TYPE_BROLY) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }
        if (this.typePet == ConstPet.TYPE_BLACK_ALT) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }
        if (this.typePet == ConstPet.TYPE_BLACK_ROSE) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }

        if (this.typePet == ConstPet.TYPE_ZAMASU) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }
        if (this.typePet == ConstPet.TYPE_CUMBER) {
            this.isTransform = !this.isTransform;
            Service.gI().Send_Caitrang(this);
            Service.gI().chat(this, "Hỡi nhân loại thấp kém! Hãy chiêm ngưỡng vẻ đẹp của ta!");
        }
    ...
    public boolean canAttack() {
        if (this.master.isPl() && this.master.doesNotAttack && this.master.charms.tdDeTu < System.currentTimeMillis()) {
            if (Util.canDoWithTime(lastTimeAskAttack, 10000)) {
                Service.gI().chatJustForMe(master, this,
                        this.typePet == ConstPet.TYPE_BLACK ? "Sao ngươi không đánh đi?" : "Sao sư phụ không đánh đi?");
            }
            return false;
        }
        return true;
    ...
    public void petSay(Player player) {
        if (this.typePet == ConstPet.TYPE_BLACK) {
            if (Util.canDoWithTime(lastTimeChat,
                    indexChat == 0 ? ConstPet.CHAT_DELAY_FIRST : ConstPet.CHAT_DELAY_NORMAL)) {
                String[] chat = {
