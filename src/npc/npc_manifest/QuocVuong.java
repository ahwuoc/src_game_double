        ...
        if (canOpenNpc(player)) {
            if (player.iDMark.isBaseMenu()) {
                    ...
                    case 0 -> {
                        if (player.nPoint.limitPower < NPoint.MAX_LIMIT) {
                            this.createOtherMenu(player, ConstNpc.OPEN_POWER_MYSEFT,
                                    ...
                                    "Nâng\ngiới hạn\nsức mạnh",
                                    "Nâng ngay\n" + Util.numberToMoney(OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER * (player.nPoint.limitPower < 10 ? 1 : 2)) + " vàng", "Đóng");
                        } else {
                ...
                switch (select) {
                    ...
                    case 1 -> {
                        if (player.inventory.gold >= OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER * (player.nPoint.limitPower < 10 ? 1 : 2)) {
                            if (OpenPowerService.gI().openPowerSpeed(player)) {
                                player.inventory.gold -= OpenPowerService.COST_SPEED_OPEN_LIMIT_POWER * (player.nPoint.limitPower < 10 ? 1 : 2);
