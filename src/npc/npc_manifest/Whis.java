...
public class Whis extends Npc {
    ...
    public void openBaseMenu(Player player) {
        if (canOpenNpc(player)) {
            switch (this.mapId) {
                case 154 -> {
                    Item BiKiepTuyetKy = InventoryService.gI().findItem(player.inventory.itemsBag, 1320);
    ...
    public void confirmMenu(Player player, int select) {
        if (canOpenNpc(player)) {
            if (player.iDMark.isBaseMenu() && (this.mapId == 154 || this.mapId == 169)) {
                ...
                switch (select) {
                    case 0 -> CombineService.gI().openTabCombine(player, CombineService.CHE_TAO_TRANG_BI_THIEN_SU);
                }
                ...
                switch (select) {
                    case 0 -> {
                        Item sach = InventoryService.gI().findItemBag(player, 1320);
