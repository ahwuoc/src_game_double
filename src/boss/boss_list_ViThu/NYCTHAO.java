...
public class NYCTHAO extends Boss {
    //fg

    private long st;

    public NYCTHAO() throws Exception {
        super(BossID.NYCTHAO, false, true, BossesData.NYCTHAO);
    }
...
public void reward(Player plKill) {
    ...
    Service.getInstance().sendThongBaoAllPlayer(
        "Bạn " + plKill.name + " nhận được " + diemsb + " điểm Săn Boss!");

    // Điểm Vĩ Thú
    int diemvithu = 1;
    plKill.point_vithu += diemvithu;
    Service.getInstance().sendThongBaoAllPlayer(
        "Bạn " + plKill.name + " nhận được " + diemvithu + " điểm Vĩ Thú!");
