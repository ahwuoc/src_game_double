...
public class NangCapKichHoatVip {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combine != null && player.combine.itemsCombine != null && player.combine.itemsCombine.size() == 1) {
            ...
            player.combine.goldCombine = 2_000_000_000;
            int goldCombie = player.combine.goldCombine;
            if (trangBiThienSu != null ) {
                ...
                Service.gI().sendThongBaoOK(player, "Cần 1 trang bị Thiên Sứ");
            }
        } else {
            Service.gI().sendThongBaoOK(player, "Cần 1 trang bị Thiên Sứ");
        }
    ...
    public static void startCombine(Player player) {
        if (player.combine.itemsCombine.size() == 1) {
            ...
            if (trangBiThienSu.template.type == 4) {
                short len = (short) Manager.radas.length;
                newItem = ItemService.gI().createNewItem((short) Manager.radas[Util.nextInt(len)] );
            } else {
                short len = (short) Manager.trangBiKichHoatVip[gender][trangBiThienSu.template.type].length;
                newItem = ItemService.gI().createNewItem(Manager.trangBiKichHoatVip[gender][trangBiThienSu.template.type][Util.nextInt(len)]);
            }
