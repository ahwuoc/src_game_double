...
public class CheTaoTrangBiThienSu {
    public static int COST = 1_000_000_000;
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (cCongThuc == 0) {
            ...
            text.append(player.inventory.gold >= COST ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Phí nâng cấp:  "+Util.numberToMoney(COST)+" vàng");
            if (manhThienSu.quantity < 999 || player.inventory.gold < COST) {
        ...
        if (cCongThuc == 1 && cManhThienSu == 1 && cDaNangCap <= 1 && cDaMayMan <= 1) {
            ...
            if (manhThienSu.quantity >= 999 && player.inventory.gold >= COST) {
                if (Util.isTrue(ratio, 100)) {
                    ...
                    InventoryService.gI().addItemBag(player, doThienSu,999999);
                } else {
