...
public class SuperRankService {
    ...
    public void competing(Player player, long id) {
        ...
        if (SuperRankManager.gI().currentlyCompeting(player)) {
            ...
            return;
        } else if (player.superRank.ticket <= 0 && player.inventory.getGemAndRuby() < 100) {
            Service.gI().sendThongBao(player, "Bạn không đủ ngọc, còn thiếu 100 ngọc nữa");
            return;
        }
        switch (menuType) {
            case 0 -> {
    ...
    public String textReward(int rank) {
        String text = "";
        if (rank == 1) {
            text = "+10.000 ruby/ ngày";
        } else if (rank >= 2 && rank <= 10) {
            text = "+5.000 ruby/ ngày";
        } else if (rank >= 11 && rank <= 100) {
            text = "+2.000 ruby/ ngày";
