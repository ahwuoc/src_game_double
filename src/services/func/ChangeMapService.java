...
public class ChangeMapService {
    ...
    public void openZoneUI(Player pl) {
        ...
        if (!pl.isAdmin()) {
            ...
            if (MapService.gI().isMapHidu(pl.zone.map.mapId)) {
                Service.gI().sendThongBaoOK(pl, "Không thể đổi khu vực trong map này");
                return;
            }
...
//            }
            ...
            if (pl.isPl() && nextMapIsMabu) {
                ...
                if (!sameZone) {
                    ...
                    if (zoneJoin.map.mapId != 114) {
                        ...
                        InventoryService.gI().addItemBag(pl, item,999999);
    ...
    public void finishLoadMap(Player player) {
        ...
        Service.gI().sendEffPlayer(player);
                Service.gI().sendEffAllPlayer(player, 151, 1, -1, 1);
            ...
            switch (zoneJoin.map.mapId) {
                ...
                case 111:
                    if(player.nPoint.power > 40_000_000_000L){
                        Service.gI().sendThongBao(player, "Yêu cầu sức mạnh dưới 40 tỉ!");
                        return null;
                    }
            }
        }
