...
public class Service {
    ...
    public void removeTitle(Player player) {
        ...
        try {
            ...
            } else if (player.inventory.itemsBody.get(13).isNotNullItem()) {
...
//    }
    ...
    public void showListTop(Player player, List<TOP> tops) {
        ...
        try {
            msg.writer().writeByte(0);
            if (tops == Manager.TopNap) {
                msg.writer().writeUTF("Top Nạp");
            }
            if (tops == Manager.Topsk) {
                msg.writer().writeUTF("Top Sự Kiện");
            }
            if (tops == Manager.TopNGUHANH) {
                msg.writer().writeUTF("Top TrainFam");
            }
            if (tops == Manager.TopPhongtap) {
                msg.writer().writeUTF("Top Phòng Tập");
            }
            if (tops == Manager.Topvithu) {
                msg.writer().writeUTF("Top Vĩ Thú");
            }
            if (tops == Manager.Topsanboss) {
                msg.writer().writeUTF("Top Săn Boss");
            }
            if (tops == Manager.topLeoThap) {
                msg.writer().writeUTF("Top Leo Tháp");
            }

            msg.writer().writeInt(Math.min(100, tops.size()));
            for (int i = 0; i < Math.min(100, tops.size()); i++) {//:))))
                TOP top = tops.get(i);
                Player pl = NDVSqlFetcher.loadById(top.getId_player());
                msg.writer().writeInt(i + 1);
                msg.writer().writeInt((int) pl.id);
...
//            e.printStackTrace();
        ...
        try {
            for (Player plMap : player.zone.getPlayers()) {
                ...
                if (plMap.isClientDoubleMessage()) {
                    ...
                    msg.writer().writeByte(0);//Hiệu ứng Ăn Đậu
                    msg.writer().writeInt(Util.maxInt(player.nPoint.hpMax));
                }
    ...
    public void Send_Info_NV_do_Injure(Player player) {
        Message msg;
        try {
            for (Player plMap : player.zone.getPlayers()) {
                msg = Service.gI().messageSubCommand((byte) 14);//Cập nhật máu
                ...
                if (plMap.isClientDoubleMessage()) {
                    msg.writer().writeDouble(player.nPoint.hp);
                    msg.writer().writeByte(2);//Hiệu ứng Ăn Đậu
                    msg.writer().writeDouble(player.nPoint.hpMax);
                } else {
                    msg.writer().writeInt(Util.maxInt(player.nPoint.hp));
                    msg.writer().writeByte(2);//Hiệu ứng Ăn Đậu
    ...
    public long exp_level1(long sucmanh) {
        if (sucmanh < 3000) {
            ...
            return 120_010_000_000L;
        } else if (sucmanh < 150_010_000_000L) {
            return 150_010_000_000L;
        }
        return 1000;
    }
        ...
        if (!player.isPet && !player.isBoss && !player.isNewPet) {
            ...
            try {
                ...
                if (player.isClientDoubleMessage()) {
                    ...
                    msg.writer().writeByte(player.nPoint.crit);// crit full
                    msg.writer().writeLong(Util.maxInt((player.nPoint.tiemNang)));
                    msg.writer().writeShort(100);
                    msg.writer().writeShort((int) player.nPoint.defg);
                    msg.writer().writeByte(player.nPoint.critg);
                }
                player.sendMessage(msg);
            } catch (Exception e) {
...
//                Logger.logException(Service.class, e);
        ...
        if (pl != null && pl.pet != null) {
            ...
            try {
                ...
                for (Item item : pl.pet.inventory.itemsBody) {
                    if (!item.isNotNullItem()) {
                        ...
                        for (ItemOption iop : item.itemOptions) {
                            ...
                            msg.writer().writeLong(iop.param);
                        }
                    }
                }
                //cso goc
                ...
                if (pl.isClientDoubleMessage()) {
                    ...
                    msg.writer().writeLong(Util.maxLong(pl.pet.nPoint.power)); //power
                    msg.writer().writeLong(Util.maxLong(pl.pet.nPoint.tiemNang)); //tiềm năng
                }
                ...
                if (pl.isClientDoubleMessage()) {
                    ...
                    msg.writer().writeInt(Util.maxInt(pl.pet.nPoint.def)); //def
                }
    ...
    public void sendTitle(Player player) {
        ...
        try {
            ...
            } else if (player.nPoint.power >= 0 && player.nPoint.power <= 100000000000l) {
...
//                if (key.equals("DragonBoy") && code == 31072002 && token.equals("DragonBoy")) {
...
//            }
    ...
    public void sendEffPlayer(Player pl) {
        if (pl.isPl()) {
            Item danhhieu = pl.inventory.itemsBody.get(13);
