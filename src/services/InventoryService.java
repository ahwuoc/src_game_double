...
import AutoGiftTop.AutoGiftTopService.AutoGiftTop;
...
public class InventoryService {
    ...
    public Item putItemBody(Player player, Item item) {
        ...
        switch (item.template.type) {
            ...
            //   case 75:
            case 72:
                index = 10;//linh thú nè
                break;
            case 77:
                index = 11;//sách tuyệt kỹ
                break;
            case 82:
                index = 12;//này nãy làm chân mệnh
                break;
            case 83:
                index = 13;//cái này nhé
                break;
            case 84:
                index = 14;
                break;
        ...
        switch (index) {
            ...
            case 7:
                ...
                break;
            case 11:
                int[] skill9 = {Skill.SUPER_KAME, Skill.MA_PHONG_BA, Skill.LIEN_HOAN_CHUONG};
                if (player.playerSkill.getSkillbyId(skill9[player.gender]) == null) {
                    Service.gI().sendThongBao(player, "Vui lòng học tuyệt kĩ trước");
                    return null;
                }
                break;
        }
        item = ItemService.gI().randomChiSoAn(item);
        item = ItemService.gI().randomChiSoAnVIP(item);
        item = ItemService.gI().randomChiSoAnHuyenao(item);
        for (int i = item.itemOptions.size() - 1; i >= 0; i--) {
            if (item.itemOptions.get(i).optionTemplate.id == 231) {
                if (Util.isTrue(97, 100)) {
                    item.itemOptions.set(i, new ItemOption(93, Util.nextInt(1, 7)));
                } else {
    ...
    public void sendEffectBody(Player player) {
        List<Item> its = player.inventory.itemsBody;
        if (its.get(7) != null && its.get(7).template != null) {
            if (player.newPet != null) {
                ChangeMapService.gI().exitMap(player.newPet);
                player.newPet.dispose();
                player.newPet = null;
            }
            PetService.Pet2(player, its.get(7).template.head, its.get(7).template.body, its.get(7).template.leg);
        }
        if (its.get(10) != null && its.get(10).template != null) {
            Service.gI().sendPetFollow(player, (short) (its.get(10).template.iconID - 1));
        }
    }
    ...
    public void itemBagToBody(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (item.template.id > 1809 && item.template.id < 1819) {
...
//            }
    ...
    public void itemBoxToBodyOrBag(Player player, int index) {
        ...
        if (item.isNotNullItem()) {
            ...
            if (!done) {
                if (addItemBag(player, item, 2000000000)) {
            ...
            for (int i = 0; i < player.inventory.itemsBag.size(); i++) {
                ...
                for (int j = 0; j < item.itemOptions.size(); j++) {
                    if (item.itemOptions.get(j).optionTemplate.id == 213) {
                        int opId = 213;
                        long param = item.itemOptions.get(j).param;
                        if (param > 1_000_000) {
                            opId = 223;
                            param /= 1_000_000;
                        } else if (param > 1000) {
                        ...
                        msg.writer().writeLong(param);
                    } else {
                        msg.writer().writeByte(item.itemOptions.get(j).optionTemplate.id);
                        msg.writer().writeLong(item.itemOptions.get(j).param);
                    }
                }
            }
                ...
                if (!item.isNotNullItem()) {
                    ...
                    for (Item.ItemOption itemOption : itemOptions) {
                        if (itemOption.optionTemplate.id == 213) {
                            ...
                            msg.writer().writeLong(itemOption.param);
                        }
                    }
                }
            }
    ...
    //    public boolean addItemBag1(Player player, Item item, double maxQuantity) {
...
//        } 
    ...
    public void addItemMail(Player plReceive, Item item) {
        try {
            Thu thu = new Thu("Vật phẩm sự kiện", item.template.name, "Được gửi bởi hệ thống", item.template.name + " x" + item.quantity, System.currentTimeMillis(), false, false);
            thu.listItem.add(item);
            plReceive.homThu.add(thu);
            HomThuService.gI().sendListMail(plReceive);
            Service.gI().sendThongBaoFromAdmin(plReceive, "Bạn nhận được thư mới");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public void addItemMail(int topId, AutoGiftTop autoTop, int plReceiveId) {
        try {
            Player plReceive = Client.gI().getPlayer(plReceiveId);
            if (plReceive == null) {
                Player plOff = NDVSqlFetcher.loadById(plReceiveId);
                if (plOff == null) {
                    System.out.println("Không tìm thấy người chơi: " + plReceiveId);
                    return;
                }
                for (Item item : autoTop.items.get(topId)) {
                    Thu thu = new Thu("Vật phẩm sự kiện", "Quà tặng TOP " + (topId + 1) + " " + autoTop.name, "Được gửi bởi hệ thống", item.template.name + " x" + item.quantity, System.currentTimeMillis(), false, false);
                    thu.listItem.add(item);
                    plOff.homThu.add(thu);
                }
                autoTop.addPlayerReceive(plReceiveId);
                System.out.println("Send Item Top " + (topId + 1) + " Successful [Player: " + plOff.name + "] [isOffline]");
            ...
            autoTop.addPlayerReceive(plReceiveId);
            System.out.println("Send Item Top " + (topId + 1) + " Successful [Player: " + plReceive.name + "] [isOffline]");
            Service.gI().sendThongBaoFromAdmin(plReceive, "Bạn nhận được thư mới");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        long[] idParam = isItemIncrementalOption(itemAdd);
        if (idParam[0] != -1) {
            for (Item it : items) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                if (!it.isNotNullItem() || it.template.id != itemAdd.template.id
                        || (!checkListsEqual(it.itemOptions, itemAdd.itemOptions) && itemAdd.template.id != 2074 && itemAdd.template.id != 1320
                        && !itemAdd.isDaNangCap())
                ...
                if ((itemAdd.template.id >= 1066 && itemAdd.template.id <= 1070) || itemAdd.template.id == 457
                        ...
                        || itemAdd.template.id == 2075 || itemAdd.template.id == 1320) {
                    it.quantity += itemAdd.quantity;
                    itemAdd.quantity = 0;
                    return true;
                }

                if (it.quantity < 2000000000) {
                    int add = 2000000000 - it.quantity;
                    if (itemAdd.quantity <= add) {
                        it.quantity += itemAdd.quantity;
                        itemAdd.quantity = 0;
                        return true;
                    } else {
                        it.quantity = 2000000000;
     ...
     */
    private long[] isItemIncrementalOption(Item item) {
        for (Item.ItemOption io : item.itemOptions) {
            switch (io.optionTemplate.id) {
                case 1:
                    return new long[]{io.optionTemplate.id, io.param};
                case 31:
                    return new long[]{io.optionTemplate.id, io.param};
            }
        }
    ...
    public long getParam(Player player, int idoption, int itemID) {
    ...
    public int getCountItemBag(Player player, int itemIdCheck) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }

    public void subQuantityItemBag(Player player, int itemIdCheck, int requiredAmount) {
        throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
    }
