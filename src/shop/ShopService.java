...
public class ShopService {
    ...
    private static final byte MAX_ITEMS_BAG = 120;
    private static final byte MAX_ITEMS_BOX = 120;
                ...
                for (TabShop tab : shop.tabShops) {
                    ...
                    for (ItemShop itemShop : tab.itemShops) {
                        ...
                        for (Item.ItemOption option : itemShop.options) {
                            ...
                            msg.writer().writeLong(option.param);
                        }
    ...
    private void buyItemBua(Player player, int itemTempId) {
        ...
        InventoryService.gI().addItemBag(player, ItemService.gI().createItemFromItemShop(is),999999);
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        if (isTuDongLuyenTap(item.template.id)) {
        ...
        if(item.itemOptions != null){
            for(int i = 0;i<item.itemOptions.size();i++){
                ...
                }else if(item.itemOptions.get(i).optionTemplate.id == 210 || item.itemOptions.get(i).optionTemplate.id == 237 ||item.itemOptions.get(i).optionTemplate.id == 238){
    ...
    public boolean isTuDongLuyenTap(int id){
        return (id == 1258 || id == 1259);
    }
    ...
    private boolean buyMoRongHanhTrang(Player player, ItemShop itemShop) {
        ...
        if (itemShop.temp.id == 518 || itemShop.temp.id == 517 || itemShop.temp.id == 1627) {
            if (itemShop.temp.id == 517 && player.inventory.itemsBag.size() >= MAX_ITEMS_BAG) {
                Service.gI().sendThongBao(player, "Đã đạt mức tối đa");
                Service.gI().sendMoney(player);
                return true;
            }
            if (itemShop.temp.id == 518 && player.inventory.itemsBox.size() >= MAX_ITEMS_BOX) {
                Service.gI().sendThongBao(player, "Đã đạt mức tối đa");
                Service.gI().sendMoney(player);
                return true;
            }
        ...
        switch (type) {
            ...
            case 2: //nhận hết
                for (int i = items.size() - 1; i >= 0; i--) {
                    item = items.get(i);
                    if (InventoryService.gI().addItemBag(player, item,999999)) {
                        Service.gI().sendThongBao(player,
                                "Bạn nhận được " + (item.template.id == 189
                                        ? Util.numberToMoney(item.quantity) + " vàng" : item.template.name));
                        items.remove(i);
        ...
        if (!item.itemOptions.isEmpty()) {
            for (ItemOption ios : item.itemOptions) {
                if (item.template.level == 14 && InventoryService.gI().optionCanUpgrade(ios.optionTemplate.id) && param > 0) {
                    ...
                    long param1 = ios.param + (ios.param * param) / 100;
